    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        spinner1.onItemSelectedListener = object: AdapterView.OnItemSelectedListener{
            override fun onItemSelected(parent:AdapterView<*>?,view: View?, position: Int, id: Long) {
                posicionSipinner1 = lista[position]
            }

            override fun onNothingSelected(p0: AdapterView<*>?) {
                Toast.makeText(this@MainActivity,"Error al traer las divisas",Toast.LENGTH_LONG).show()
            }
        }
        spinner2.onItemSelectedListener = object: AdapterView.OnItemSelectedListener{
            override fun onItemSelected(parent:AdapterView<*>?,view: View?, position: Int, id: Long) {
                posicionSpinner2 = lista[position]
            }
        ...
        binding.BtnEnviarConvertir.setOnClickListener {
            ...
            if (cantidad.text.isNotEmpty()){
                when(posicionSipinner1){
                    "Pesos mexicanos"->{
                        when(posicionSpinner2){
                            "Pesos mexicanos"->{
                                resultado.text = cantidad.text
                            }
                            "Dolares"->{
                                total =cantidad.text.toString().toFloat()*0.4
                    ...
                    "Euros"->{
                        when(posicionSpinner2){
                            "Pesos mexicanos"->{
                                total = cantidad.text.toString().toFloat()* 21.64
                                resultado.text=total.toString()                            }
                            "Dolares"->{
